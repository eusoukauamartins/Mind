...
import { Plus, DollarSign, TrendingUp, TrendingDown, Search, Trash2, Edit2, Wallet, Edit3, LineChart as LineChartIcon, X, Upload } from 'lucide-react';
import { formatCurrency, formatDate, isThisMonth, isThisWeek, getWeekDates } from '../utils/helpers';
import { PieChart, Pie, Cell, BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, Legend } from 'recharts';
import FixedCosts from '../components/FixedCosts';
import FinanceImportReview from '../components/FinanceImportReview';
...
export default function Finance() {
  ...
  const [initialBalance, setInitialBalance] = useState(() => getSafeInitialBalance());
  ...
  // Update initialBalance state whenever finance collection changes
  useEffect(() => {
    setInitialBalance(getSafeInitialBalance());
  }, [finance]);
  ...
  const handleUpdateInitialBalance = (e) => {
    ...
    if (val !== null && val.trim() !== '') {
      ...
      if (!isNaN(parsed)) {
        ...
        saveSetting('cp_initial_balance', parsed);
      }
    }
  };
  ...
  useEffect(() => {
    ...
    if (location.state?.scrollToFixedCosts) {
      setTimeout(() => {
        const el = document.getElementById('fixed-costs-section');
        if (el) {
  ...
  const filtered = useMemo(() => {
    ...
    if (dateFilter.start && dateFilter.end) {
      result = result.filter(f => f && typeof f.date === 'string' && f.date >= dateFilter.start && f.date <= dateFilter.end);
    }
    result.sort((a, b) => {
  ...
  const metrics = useMemo(() => {
    const income = filtered.filter(f => f && f.type === 'entrada').reduce((s, f) => s + (Number(f.amount) || 0), 0);
    const expenses = filtered.filter(f => f && f.type === 'saída').reduce((s, f) => s + (Number(f.amount) || 0), 0);
    ...
    filtered.forEach(f => {
      ...
      categoryMap[key].value += (Number(f.amount) || 0);
    });
    ...
    filtered.forEach(f => {
      ...
      sourceMap[key].value += (Number(f.amount) || 0);
    });
    ...
    const allChronological = [...finance]
      .filter(f => f && typeof f.date === 'string')
      .sort((a, b) => a.date.localeCompare(b.date));
    let runningAccumulator = initialBalance;
    const dateBalances = {}; 
    allChronological.forEach(f => {
      if(f.type === 'entrada') runningAccumulator += (Number(f.amount) || 0);
      else if(f.type === 'saída') runningAccumulator -= (Number(f.amount) || 0);
  ...
  const itemsToRender = useMemo(() => {
    if (isMobile) {
      if (mobileVisibleCount < 50) {
        return filtered.slice(0, mobileVisibleCount);
      } else {
        return filtered.slice(mobilePage * 50, (mobilePage + 1) * 50);
      }
    }
    return filtered.slice(0, visibleCount);
  }, [filtered, isMobile, mobileVisibleCount, mobilePage, visibleCount]);

  const handleSubmit = () => {
    if (!form.amount || !form.originalDescription) return;
  ...
  return (
    <div className="page-container">
        ...
        {filtered.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            {itemsToRender.map(entry => (
              <div key={entry.id} className="transaction-row" style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 500, fontSize: 'var(--fs-base)' }}>{entry.originalDescription || entry.category || 'Sem nome'}{entry.expenseClass && entry.type === 'saída' ? ` — ${entry.expenseClass}` : ''}</div>
            ...
            {isMobile ? (
              mobileVisibleCount < 50 ? (
                ...
                filtered.length > 50 && (
                  <div style={{ padding: 'var(--sp-4)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid var(--border)', gap: 'var(--sp-4)' }}>
                    <button 
                    ...
                    <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)' }}>
                      Pág. {mobilePage + 1} de {Math.ceil(filtered.length / 50)}
                    </span>
                    <button 
                      className="btn btn-secondary btn-sm" 
                      onClick={() => setMobilePage(p => Math.min(Math.ceil(filtered.length / 50) - 1, p + 1))}
                      disabled={mobilePage >= Math.ceil(filtered.length / 50) - 1}
                    >
                      Próximo
                    </button>
                  </div>
      ...
      {showModal && (
        <Modal title={editing ? 'Editar Lançamento' : 'Novo Lançamento'} onClose={() => { setShowModal(false); setEditing(null); }}>
          <div className="form-group">
            ...
            <input className="form-input" placeholder="Ex: Uber, Mercado, Cliente X" value={form.originalDescription} onChange={e => setForm({ ...form, originalDescription: e.target.value })} autoFocus />
          </div>
      ...
      {showImportModal && (
        <FinanceImportReview onClose={() => setShowImportModal(false)} />
      )}
    </div>
  );
}
