...
import { mapLocalToRemoteLearning, mapLocalToRemoteExperiment, mapLocalToRemoteCheckin, mapLocalToRemoteAllocation } from '../lib/batch1Sync';
import { mapLocalToRemoteRoutine, mapLocalToRemoteLog } from '../lib/batch2Sync';
import { mapLocalToRemoteProject } from '../lib/batch3Sync';
...
const modules = [
  ...
  { key: 'fixedCosts', label: 'Custos Fixos', icon: DollarSign, stateKey: 'fixedCosts' },
  { key: 'rewards', label: 'Recompensas', icon: Gift, stateKey: 'rewards' },
];
...
export default function Config() {
  ...
  const [fullImportPreview, setFullImportPreview] = useState(null); // { normalizedData, presentKeys, warnings, fileName }
  ...
  const handleModeChange = (mode) => {
    setCurrentMode(mode);
    saveSetting('cp_mode', mode);
    document.documentElement.setAttribute('data-mode', mode);
  };

  const handleAccentChange = (accentId) => {
    setCurrentAccent(accentId);
    saveSetting('cp_accent', accentId);
    document.documentElement.setAttribute('data-accent', accentId);
  };

  const handleResetData = async () => {
    if (window.confirm('Tem certeza? Isso apagará TODOS os seus dados permanentemente (local E nuvem). Essa ação não pode ser desfeita.')) {
      await resetAllData(appState.refreshAll);
      alert('Dados apagados com sucesso (local e nuvem).');
    }
  };
  ...
  const handleExport = () => {
    ...
    if (format === 'json') {
      const exportData = {};
      // Export ALL modules for complete backup
      modules.forEach(mod => {
        const data = appState[mod.stateKey] || [];
        exportData[mod.key] = data;
      });
      exportData._metadata = {
        exportado_em: new Date().toISOString(),
        app: 'Lyria',
        versao_backup: '1.0',
        modulos: modules.map(m => m.key),
        periodo: { inicio: 'todos', fim: 'todos' },
      };

      // Collect settings keys, excluding database collections keys to avoid duplicate writes
      const settingsData = {};
      const collectionKeys = Object.values(dbKeys).map(k => 'cp_' + k);
      ...
      const dynamicSettings = Object.keys(localStorage).filter(k => 
        k.startsWith('cp_') && 
        !k.includes('initialized') && 
        !k.includes('_demo_loaded') &&
        !collectionKeys.includes(k) &&
        !explicitSettings.includes(k)
      );

      const allSettingsKeys = [...explicitSettings, ...dynamicSettings];
      ...
      exportData._settings = settingsData;

      // Preserve omitted "Ignorar" finance entries at the top level for visibility.
      const omittedRaw = localStorage.getItem('cp_omitted_finance_ignorar');
      if (omittedRaw) {
        try {
          const parsedOmitted = JSON.parse(omittedRaw);
          if (Array.isArray(parsedOmitted) && parsedOmitted.length > 0) {
            exportData._omitted_finance_ignorar = parsedOmitted;
          }
  ...
  const handleImport = (e) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        // Accept if it has valid keys or app metadata
        const hasValidKeys = Object.keys(rawData).some(key => {
          const lowerKey = key.toLowerCase();
          return [
            'tasks', 'finance', 'finance_entries', 'fixedcosts', 'fixed_costs', 'projects',
            'learnings', 'experiments', 'weeklyreviews', 'weekly_reviews', 'dailycheckins',
            'daily_checkins', 'timeallocations', 'time_allocations', 'workoutroutines',
            'workout_routines', 'workoutlogs', 'workout_logs', '_settings', '_metadata'
          ].includes(lowerKey) || key.startsWith('cp_') || key === 'lyria_quote_unlocked_date';
        });

        if (!hasValidKeys) {
          alert('Arquivo inválido. O arquivo selecionado não é um backup válido.');
          e.target.value = '';
          return;
        }

        const { normalized, presentKeys, warnings } = normalizeBackupData(rawData);

        setFullImportPreview({
  ...
  const handleConfirmFullImport = async () => {
    ...
    const { normalizedData } = fullImportPreview;

    try {
      // Build mappers object for importFullBackup
      const mappers = {
        tasks: mapLocalToRemoteTask,
        learnings: mapLocalToRemoteLearning,
        experiments: mapLocalToRemoteExperiment,
        dailyCheckIns: mapLocalToRemoteCheckin,
        timeAllocations: mapLocalToRemoteAllocation,
        workoutRoutines: mapLocalToRemoteRoutine,
        workoutLogs: mapLocalToRemoteLog,
        projects: mapLocalToRemoteProject,
        finance: mapLocalToRemoteFinance,
        fixedCosts: mapLocalToRemoteFixedCost,
        rewards: mapLocalToRemoteReward,
        weeklyReviews: mapLocalToRemoteWeeklyReview,
      };

      await importFullBackup(normalizedData, mappers, appState.refreshAll);
      setFullImportPreview(null);
      alert('Backup restaurado com sucesso! Dados sincronizados local e remotamente.');
      window.location.reload();
    } catch (err) {
      console.error('[Lyria Import] Error restoring backup:', err);
      alert(err.message || 'Ocorreu um erro ao restaurar os dados do backup.');
    }
  };
    ...
    reader.onload = (event) => {
      try {
        ...
        mergeableModules.forEach(mod => {
          const incoming = normalized[mod.key];
        ...
        setMergePreview({ summary, rawData: normalized, fileName: file.name });
      } catch (err) {
        alert('Erro ao ler o arquivo JSON. Ele pode estar corrompido ou mal formatado.');
      }
      e.target.value = '';
  ...
  const handleConfirmMerge = () => {
    ...
    mergeableModules.forEach(mod => {
      ...
      if (toAdd.length > 0) {
        ...
        const dbKey = dbKeys[mod.stateKey] || mod.stateKey;
        db.set(dbKey, merged);
      }
    });
  ...
  return (
    <div className="page-container" style={{ maxWidth: 900 }}>
      ...
      {/* Full Import Preview Modal */}
      {fullImportPreview && (
        <Modal title="Confirmar Importação de Backup" onClose={() => setFullImportPreview(null)}>
          <p style={{ color: 'var(--text-secondary)', fontSize: 'var(--fs-sm)', marginBottom: 'var(--sp-4)' }}>
            Arquivo: <strong style={{ color: 'var(--text-primary)' }}>{fullImportPreview.fileName}</strong>
          </p>

          {fullImportPreview.warnings.length > 0 && (
            <div style={{
              ...
              {fullImportPreview.warnings.map((warn, idx) => (
          ...
          <div style={{ marginBottom: 'var(--sp-4)' }}>
            <h4 style={{ fontSize: 'var(--fs-sm)', color: 'var(--text-primary)', marginBottom: 'var(--sp-3)', fontWeight: 600 }}>
              Módulos Detectados:
            </h4>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-2)' }}>
              {modules.map(mod => {
                const detected = fullImportPreview.presentKeys[mod.key];
                if (!detected) return null;
                const count = (fullImportPreview.normalizedData[mod.key] || []).length;
                return (
                  <div key={mod.key} style={{
                    ...
                    <span style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-2)', fontSize: 'var(--fs-sm)', color: 'var(--text-primary)' }}>
                      {mod.icon && <mod.icon size={16} className="text-muted" />}
                      {mod.label}
                    </span>
          ...
          <div style={{
            ...
            <strong>Atenção:</strong> Esta ação irá apagar permanentemente todos os dados atuais locais e da nuvem antes de restaurar o backup.
          </div>

          <div className="form-actions" style={{ display: 'flex', justifyContent: 'flex-end', gap: 'var(--sp-3)' }}>
            <button className="btn btn-secondary" onClick={() => setFullImportPreview(null)}>
              Cancelar
            </button>
            <button className="btn btn-primary" onClick={handleConfirmFullImport}>
